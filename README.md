...
- **`search_literature` ranking is relevance-first, not citation-first.** Sorting merged results by citation count alone surfaced clinical guidelines and burden-of-disease reviews (8,000+ citations) above the pivotal randomised trial a query was actually about. Results now sort by corroboration, then each index's own relevance rank, then citations. Measured on one query: the pivotal trial moved from rank 6 to rank 1. Recall of mid-list papers is still bounded by what each index ranks highly — expand the query rather than raising `limit` alone.
