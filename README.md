...
- **Retraction screening always reports itself, including when it finds nothing.** `search_literature` and `expand_citations` return a `retraction_screening` block that is never omitted: how many records were screened, how many actually carried a status field, and what was flagged. A screen that ran and found nothing and a screen that never ran emit identical output when only the absence of hits is reported, and there is then nothing to audit the null against. `records_carrying_status_data` is the positive control — zero flags together with zero records carrying the field is reported as **uninformative, not clean**, which is what a Crossref-only search returns, since Crossref does not supply the field.
...
- **Full text comes from PMC, so effectively biomedical and life sciences.** A physics paper will usually return `no_pmc_record`; use `resolve_fulltext` and read the arXiv copy. `fetch_fulltext` tries Europe PMC first and then NCBI E-utilities, because the two gate the same corpus differently: Europe PMC's `isOpenAccess` flag is stricter than "readable" and refuses records NCBI serves in full. Measured on three papers Europe PMC declined, NCBI returned full body text for two. The third exists only as a 1992 scan and returns `abstract_only` — reported as **not retrieved**, because an abstract presented as full text is exactly the access inflation `identity.md` rule 5 forbids.
- **No structural/coordinate retrieval.** The server resolves PDB identifiers through metadata but never downloads coordinates, so no distance, contact or buried-surface calculation is possible from its output. A comparison against a system that does compute over structures showed this is the largest remaining capability gap for structural-biology work.
